...
class MobileControls extends FlxGroup
{
	...
	public function new(dpad:Int, actions:Int, camera:FlxCamera = null)
	{
		super();
		buttonCam = camera;
		createDpad(dpad);
	...
	function createButton(x:Float, y:Float, img:String, key:FlxKey)
	{
		...
		btn.loadGraphic("assets/images/mobile/buttons/" + img.toUpperCase() + ".png");
		btn.scrollFactor.set(0,0);
		if(buttonCam != null) btn.cameras = [buttonCam];
		btn.alpha = 0.5;
	...
	{
		var eventType = if(pressed) KeyboardEvent.KEY_DOWN else KeyboardEvent.KEY_UP;
