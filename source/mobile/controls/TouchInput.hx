...
class TouchInput {
    ...
    public static function inArea(x:Float, y:Float, width:Float, height:Float):Bool {
        #if mobile
        ...
        for (swipe in FlxG.swipes) {
            if (swipe != null && swipe.duration >= 0) {
                ...
                switch (direction.toLowerCase()) {
                    case "down": if (deltaY < -30 && Math.abs(deltaY) > Math.abs(deltaX)) return true;
                    case "up": if (deltaY > 30 && Math.abs(deltaY) > Math.abs(deltaX)) return true;
                    case "right": if (deltaX < -30 && Math.abs(deltaX) > Math.abs(deltaY)) return true;
                    case "left": if (deltaX > 30 && Math.abs(deltaX) > Math.abs(deltaY)) return true;
